...
const Collegepage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    ;(async () => {
      try {
        // send to backend
        const saved = await request('/api/reviews', { method: 'POST', body: payload })
  ...
  useEffect(() => {
    ...
    const loadCollege = async () => {
      ...
      try {
        ...
        if (mounted) setFormData((p) => ({ ...p, collegeName: data.name || '' }))
      } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-md p-8">
        <h2 className="text-3xl font-bold text-gray-800 mb-6">Write a Review</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder={collegeId ? 'Loading college...' : 'Enter college name'}
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((rating) => (
                ...
                >
                  <span className={`text-3xl ${rating <= formData.rating ? "text-yellow-400" : "text-gray-300"}`}>
                    ★
                  </span>
                </button>
              ))}
